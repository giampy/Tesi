...
Si pu\`o dire che questa reazione \`e un adattamento della nozione di equivalenza osservazionale (del modello formale) in modo che corrisponda all'indistinguibilit\`a del modello computazionale.
Se due processi sono legati da questa relazione d'equivalenza allora sono equivalenti a meno di una certa probabilit\`a nel senso che, qualsiasi attaccante (contesto) $C$ che si esegue per un tempo $t$
ha una probabilit\`a di distinguere questi due processi al massimo $p(t)$.
...
Come \`e stato detto le regole di trasformazione da un game ad un altro sono date mediante un'equivalenz. Dato un game $G$, per utilizzare un generica regola di trasformazione  $L \approx_p R$,
CryptoVerif trova un contesto $C$ tale che $G \approx_0 C[L] $ semplicemente apportando delle modifiche sintattiche al game $G$, costruisce inoltre un game $G^{'}$
tale che $G^{'} \approx_0 C[R]$. Adesso CryptoVerif grazie alla propriet\`a 3 pu\`o dedurre che $G \approx_p G^{'}$. Con questa tecnica CryptoVerif riesce spesso a costruire catene
