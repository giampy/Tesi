...
\hyphenation{li-mi-ta-te li-mi-ta-ti e-qui-va-len-ti ov-via-men-te pseudo-ca-sua-le sem-bre-reb-be mo-di-fi-che}
\floatstyle{boxed} 
\restylefloat{figure}
...
Da quando negli anni anni '80  ha cessato di essere un'arte per assurgere al rango di scienza, la crittografia moderna
si \`e proposta innanzi tutto di fornire definizioni rigorose dei concetti con cui aveva e ha a che fare (e.g. \emph{segretezza}) per poi apportare prove dei propri asserti basandosi 
...
Procediamo adesso con una breve spiegazione di questo gioco.
...
\begin{verbatim}       
 ...
 \end{verbatim}
Per ora, non \`e interessante per quale probabilit\`a questi giochi siano equivalenti, ecco perch\`e si \`e semplicemente indicato il valore della probabilit\`a con $pow$ senza specificare ulteriormente.
...
Si \`e dimostrato che CryptoVerif pu\`o provare che, per $k=2$, se si ha un generatore pseudocasuale $\texttt{G'}$ con coefficiente d'espansione $n+1$ 
allora si ha anche un generatore pseudocasuale con coefficiente d'espansione $n+k$. Adesso bisogna anche effettuare il passo induttivo. 
...
Questa dimostrazione proceder\`a fornendo uno scheletro di un gioco iniziale che serve a dimostrare l'asserto per $n+i+1$ e provando che questo gioco si riduce in un numero di passi costante 
al gioco che sarebbe servito per dimostrare
l'asserto per $n+i$ che si suppone corretto per ipotesi induttiva.
Il gioco iniziale per sar\`a caratterizzato da:
\begin{itemize}
 \item dichiarazione del tipo di base: $\texttt{type npbits [fixed].}$
 \item $i$ dichiarazioni di tipo: \begin{itemize}
                                       \item $\texttt{type np1bits [fixed].}$
                                       \item $\texttt{type np2bits [fixed].}$
				       \item \dots
				       \item $\texttt{type npibits [fixed].}$
                                      \end{itemize}
  \item $i$ dichiarazioni di simobli funzionali del tipo:\begin{itemize}
  ...
  \item  $n+i+1$ equivalenze del tipo:
				\begin{itemize}
				 \item \begin{verbatim}
equiv
     foreach i1<=n1 do
     ...
     <=(0)=>
      foreach i1<=n1 do
             ...
             Oget():=return(w).
				\end{verbatim}
  Quando $i=0$ il tipo da cui viene estratto il valore assegnato ad r \`e $\texttt{nbits}$
\end{itemize}

\item le dichiarazioni dei seguenti simboli funzionali:
      \begin{itemize}
       \item $\texttt{fun getn(np1bits):nbits.}$
       \item $\texttt{fun getlast(np1bits):bool.}$
      \end{itemize}		
      ...
      \begin{itemize}
       \item \begin{verbatim}
        ...
        foreach i1 <=n1 do
             ...
             \end{verbatim}
      \end{itemize}
\item la dichiarazione del generatore pseudocasuale con coefficiente d'espansione $n+1$:
\begin{itemize}
 \item \begin{verbatim}
        fun G'(nbits): np1bits.
       \end{verbatim}
...
\item la clausola: $\texttt{query secret w.}$
...
\begin{itemize}
 ...
 process
        O():=
              ...
              ...
	      let xi=G'(y(i-1)) in
              let yi=getn(x(i-1) in
              let bi=getlast(x(i-1)) in
w:np(i+1)bits <-concatnpi(concatnpi-1....(concatnp1(G'(yi), b1), b2), .....)bi);
return()
       \end{verbatim}
\end{itemize}
\end{itemize}

Questo \`e uno scheletro di ogni input necessario a dimostrare la validit\`a dell'asserto per qualche $k=n+i+1$ mediante CryptoVerif. Se si analizzano le ultime 5 righe si vede che la linea: 
\begin{verbatim}
 let xi=G'(y(i-1)) in
\end{verbatim}

attraverso le trasformazioni dettate dalle definizioni di equivalenza sopra citate si arriva ad una situazione del genere:
\begin{verbatim}
w: np4bits <- concatnp(n+i)(concatnp(n+i-1)(w_21, xl_16), xl_19);
\end{verbatim}
il primo argomento all'interno della funzione \`e semplicemente il risultato della dimostrazione per $n+i$. Che \`e vera per per ipotesi induzione e quindi arriviamo a questa situazione:
\begin{verbatim}
w: np4bits <- concatnp(n+i)(w_1, xl_19);
\end{verbatim}
ora grazie al fatto che la concatenazione di due stringhe random \`e una stringa random si ha la tesi.
\chapter{Conclusioni}
