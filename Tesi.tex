...
Sebbene siano stati ideati cifrari sicuri anche rispetto ad avversari non limitati\footnote{Il cifrario \emph{one-time pad} \`e il tipico esempio di cifrario perfettamente sicuro.}, questi hanno alcuni difetti come per esempio
...
\begin{itemize}
 \item l'avversario non conosce la chiave, ma conosce l'algoritmo di cifratura utilizzato  e i parametri di sicurezza, come per esempio la lunghezza della chiave\footnote{Il principio di Kerchoffs, famoso crittografo olandese (19 Gennaio 1835 - 9 Agosto 1903) afferma che,
...
Nel modello computazionale i messaggi sono trattate come sequenze di bit, e non come espressioni \emph{formali}; l'avversario, nel modello computazionale, pu\`o effettuare qualsiasi operazione su un messaggio. Questa visione \`e, a differenza di quella 
...
\end{quotation}
Potrebbe sembrare pericoloso utilizzare degli schemi crittografici che ammettono di essere violati con probabilit\`a trascurabile, ma questa possibilit\`a \`e cosi remota, che una tale preoccupazione \`e da ritenersi
ingiustificata.
...
Nella pratica lo scopo di $G$ \`e prendere in input un $seed$ casuale, e da quello generare una variabile pseudocasuale molto pi\`u lunga. Si inutisce da questo la grandissima
...
Quella basata sul concetto di game \`e una tecnica\footnote{Game hopping tecnique.} molto utilizzata per provare la sicurezza di primitive crittografiche o di protocolli crittografici.
...
\`e computazionalmente indistinguibile dal penultimo, e il penultimo lo \`e dal terzultimo e il terzultimo lo \`e dal quartultimo e cosi via fino a $G_0$, allora
...
Nel modello formale questa relazione sar\`a l'indistinguibilit\`a perfetta (meglio nota come equivalenza osservazionale) mentre in quello computazionale sar\`a l'indistinguibilit\`a computazionale.
\chapter{CryptoVerif}
...
Abbiamo poi il processo $Ogen()$ che ritorna al contesto una chiave pubblica dopo aver creato, partendo da un seme casuale, una coppia (chiave privata, chiave pubblica).
