...
Un contesto \`e un processo con un $buco$, ovvero un processo che prende come argomento un altro processo e viene eseguito in parallelo a questo. Di solito un contesto \`e un attaccante che cerca di distinguere un processo che 
...
Se $a$ \`e una qualsiasia stringa (supponiamo binaria per semplicit\`a) $C$ \`e un contesto e $Q$ un processo, allora con la notazione $Pr[C[Q] \rightsquigarrow a]$ si intende la probabilit\`a
che il contesto (ovvero l'osservatore) $C$ riesca ad osservare $a$ come output di $Q$. \\
Se $Q$ \`e un processo e $\varepsilon$ \`e una sequenza ordinata di eventi allora con i simboli $Q \rightsquigarrow \varepsilon$ si indica che $Q$ esegue la sequenza ordinata di eventi $\varepsilon$ . 
\newpage
...
$\lvert Pr[C[Q] \rightsquigarrow a] - Pr[C[Q^{'}]\rightsquigarrow a] \rvert \leq p(t)$ $\lvert \rvert \leq p(t)$ 
\end{definizione}
 
Si pu\`o dire che questa relazione sia un adattamento della nozione di equivalenza osservazionale (del modello formale) in modo che corrisponda all'indistinguibilit\`a del modello computazionale 
nel momento in cui la probabilit\`a che permette ai due processi di essere in relazione \`e trascurabile. Se due processi sono legati da questa relazione d'equivalenza allora sono equivalenti a meno di una certa probabilit\`a nel senso che, 
qualsiasi attaccante (contesto) $C$ che si esegue per un tempo $t$ ha una probabilit\`a di distinguere questi due processi al massimo $p(t)$.
...
osservazionalmente equivalenti (secondo la definizione di cui prima) fra loro: $G_0 \approx_{p_1} G_1 \approx_{p_2} G_2 \dots \approx_{p_f} G_f $; data questa sequenza, 
attraverso le propriet\`a 1 e 2, conclude che $G_0  \approx_{\sum_{p=1}^{f}} G_f $, riuscendo cos\`i a dare un limite alla probabilit\`a che un attaccante riesca a distinguere
$G_0$ da $G_f$.\\
Come \`e stato spiegato precedentemente le regole di trasformazione da un game ad un altro sono date mediante delle equivalenza. 
...
apportando delle modifiche sintattiche al game $G$, costruisce inoltre un game $G^{'}$ tale che $G^{'} \approx_0 C[R]$. Adesso CryptoVerif grazie alla propriet\`a 3 pu\`o dedurre che $G \approx_p G^{'}$. Con questa tecnica CryptoVerif riesce spesso a costruire catene
